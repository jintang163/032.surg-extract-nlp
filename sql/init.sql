...
-- 18. ���������
-- -----------------------------------------------------------
DROP TABLE IF EXISTS atch_task;
CREATE TABLE atch_task (
    id                  BIGINT          NOT NULL AUTO_INCREMENT COMMENT '����ID',
    	ask_name           VARCHAR(128)    NOT NULL COMMENT '��������',
    department          VARCHAR(128)    DEFAULT NULL COMMENT '�������',
    	ask_type           VARCHAR(32)     NOT NULL DEFAULT 'SURGERY_RECORD' COMMENT '��������: SURGERY_RECORD-������¼',
    original_file_name  VARCHAR(255)    DEFAULT NULL COMMENT 'ԭʼZIP�ļ���',
    ile_path           VARCHAR(512)    DEFAULT NULL COMMENT 'ZIP�ļ��洢·��',
    ile_size           BIGINT          DEFAULT 0 COMMENT 'ZIP�ļ���С(�ֽ�)',
    	otal_count         INT             NOT NULL DEFAULT 0 COMMENT '���ļ���',
    success_count       INT             NOT NULL DEFAULT 0 COMMENT '�ɹ���',
    ailed_count        INT             NOT NULL DEFAULT 0 COMMENT 'ʧ����',
    pending_count       INT             NOT NULL DEFAULT 0 COMMENT '��������',
    status              VARCHAR(32)     NOT NULL DEFAULT 'PENDING' COMMENT '����״̬: PENDING-������, PROCESSING-������, COMPLETED-�����, FAILED-ʧ��',
    error_message     VARCHAR(512)    DEFAULT NULL COMMENT '������Ϣ',
    
otify_type         VARCHAR(32)     NOT NULL DEFAULT 'EMAIL' COMMENT '֪ͨ����: EMAIL-�ʼ�, WECHAT-΢��, ALL-ȫ��',
    
otify_target     VARCHAR(255)    DEFAULT NULL COMMENT '֪ͨĿ��(�����΢��OpenID)',
    
otified            TINYINT         NOT NULL DEFAULT 0 COMMENT '�Ƿ���֪ͨ: 0-��, 1-��',
    
etry_count         INT             NOT NULL DEFAULT 0 COMMENT '���Դ���',
    max_retry_count     INT             NOT NULL DEFAULT 3 COMMENT '������Դ���',
    ...
    created_time        DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '����ʱ��',
    updated_time        DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '����ʱ��',
...
-- 19. ����������ϸ��
-- -----------------------------------------------------------
DROP TABLE IF EXISTS atch_task_item;
CREATE TABLE atch_task_item (
    id                  BIGINT          NOT NULL AUTO_INCREMENT COMMENT '����ID',
    	ask_id           BIGINT          NOT NULL COMMENT '��������ID(����batch_task)',
    ile_name        VARCHAR(255)    NOT NULL COMMENT '�ļ���',
    ile_path        VARCHAR(512)    DEFAULT NULL COMMENT '�ļ��洢·��',
    ile_type         VARCHAR(32)     NOT NULL COMMENT '�ļ�����: TEXT, WORD, PDF, IMAGE',
    patient_name    VARCHAR(64)     DEFAULT NULL COMMENT '��������',
    hospital_no     VARCHAR(64)     DEFAULT NULL COMMENT 'סԺ��',
    
ecord_id         BIGINT          DEFAULT NULL COMMENT '������������¼ID',
    status            VARCHAR(32)     NOT NULL DEFAULT 'PENDING' COMMENT '����״̬: PENDING-������, PROCESSING-������, SUCCESS-�ɹ�, FAILED-ʧ��',
    error_message   VARCHAR(512)    DEFAULT NULL COMMENT '������Ϣ',
...
ecord_id               BIGINT          NOT NULL COMMENT '手术记录ID',
    
ecord_no               VARCHAR(64)     DEFAULT NULL COMMENT '手术记录编号',
    ...
    original_source         VARCHAR(32)     DEFAULT NULL COMMENT '原始来源',
    ...
    correction_type         VARCHAR(32)     DEFAULT NULL COMMENT '修正类型',
    eedback_source         VARCHAR(32)     DEFAULT NULL COMMENT '反馈来源',
    department              VARCHAR(128)    DEFAULT NULL COMMENT '科室',
    eedback_user_id        BIGINT          DEFAULT NULL COMMENT '反馈人ID',
    eedback_user_name      VARCHAR(64)     DEFAULT NULL COMMENT '反馈人姓名',
    eedback_remark         VARCHAR(512)    DEFAULT NULL COMMENT '反馈备注',
    quality_score           INT             DEFAULT NULL COMMENT '质量评分 0-100',
    used_for_training       TINYINT         NOT NULL DEFAULT 0 COMMENT '是否已用于训练',
    used_time               DATETIME        DEFAULT NULL COMMENT '使用时间',
    	rain_batch_no          VARCHAR(64)     DEFAULT NULL COMMENT '训练批次号',
    created_time            DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_time            DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
ecord_id),
    ...
    KEY idx_created_time (created_time),
    KEY idx_feedback_user_id (eedback_user_id),
    KEY idx_department (department)
...
CREATE TABLE model_train_log (
    id                      BIGINT          NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    	rain_batch_no          VARCHAR(64)     NOT NULL COMMENT '训练批次号',
    model_name              VARCHAR(128)    DEFAULT NULL COMMENT '模型名称',
    model_version           VARCHAR(64)     DEFAULT NULL COMMENT '模型版本号',
    previous_version        VARCHAR(64)     DEFAULT NULL COMMENT '上一版本号',
    	rain_type              VARCHAR(32)     NOT NULL DEFAULT 'INCREMENTAL' COMMENT '训练类型',
    eedback_count          INT             DEFAULT 0 COMMENT '使用的反馈数据量',
    
ew_sample_count        INT             DEFAULT 0 COMMENT '新增训练样本数',
    	otal_sample_count      INT             DEFAULT 0 COMMENT '总训练样本数',
    	rain_loss              DECIMAL(10,6)   DEFAULT NULL COMMENT '训练损失',
    dev_loss                DECIMAL(10,6)   DEFAULT NULL COMMENT '验证损失',
    precision_score         DECIMAL(5,4)    DEFAULT NULL COMMENT '精确率',
    
ecall_score            DECIMAL(5,4)    DEFAULT NULL COMMENT '召回率',
    1_score                DECIMAL(5,4)    DEFAULT NULL COMMENT 'F1分数',
    previous_f1_score       DECIMAL(5,4)    DEFAULT NULL COMMENT '上一版本F1分数',
    1_improvement          DECIMAL(5,4)    DEFAULT NULL COMMENT 'F1提升幅度',
    entity_type_breakdown   TEXT            DEFAULT NULL COMMENT '各实体类型指标明细(JSON)',
    	rain_status            VARCHAR(32)     NOT NULL DEFAULT 'PENDING' COMMENT '训练状态',
    ail_reason             VARCHAR(512)    DEFAULT NULL COMMENT '失败原因',
    	rain_start_time        DATETIME        DEFAULT NULL COMMENT '训练开始时间',
    	rain_end_time          DATETIME        DEFAULT NULL COMMENT '训练结束时间',
    	rain_duration_sec      INT             DEFAULT NULL COMMENT '训练耗时(秒)',
    	rain_params            TEXT            DEFAULT NULL COMMENT '训练参数(JSON)',
    	riggered_by            BIGINT          DEFAULT NULL COMMENT '触发人ID',
    	riggered_by_name       VARCHAR(64)     DEFAULT NULL COMMENT '触发人姓名',
    model_path              VARCHAR(512)    DEFAULT NULL COMMENT '模型存储路径',
    
emark                  VARCHAR(512)    DEFAULT NULL COMMENT '备注',
    created_time            DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_time            DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted                 TINYINT         NOT NULL DEFAULT 0 COMMENT '逻辑删除',
    PRIMARY KEY (id),
    UNIQUE KEY uk_train_batch_no (	rain_batch_no),
    KEY idx_train_status (	rain_status),
    KEY idx_train_start_time (	rain_start_time)
...
CREATE TABLE export_template (
    id                      BIGINT          NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    	emplate_name           VARCHAR(128)    NOT NULL COMMENT '模板名称',
    	emplate_code           VARCHAR(64)     DEFAULT NULL COMMENT '模板编码',
    description             VARCHAR(512)    DEFAULT NULL COMMENT '模板描述',
    export_format           VARCHAR(32)     NOT NULL COMMENT '导出格式: EXCEL-Excel, JSON-JSON, FHIR-HL7 FHIR',
    	arget_system           VARCHAR(64)     DEFAULT NULL COMMENT '目标系统: HIS/EMR/OTHER',
    department              VARCHAR(128)    DEFAULT NULL COMMENT '适用科室',
    ield_configs           TEXT            DEFAULT NULL COMMENT '字段配置(JSON数组)',
    unit_conversions        TEXT            DEFAULT NULL COMMENT '单位转换配置(JSON数组)',
    sort_order              INT             DEFAULT 0 COMMENT '排序',
    ...
    deleted                 TINYINT         NOT NULL DEFAULT 0 COMMENT '逻辑删除',
    PRIMARY KEY (id),
    UNIQUE KEY uk_template_code (	emplate_code),
...
-- 插入默认导出模板
-- -----------------------------------------------------------
INSERT INTO export_template (template_name, template_code, description, export_format, target_system, is_default, field_configs) VALUES
('病案首页-标准Excel', 'HOMEPAGE_EXCEL_STD', '病案首页标准Excel导出格式，包含全部标准化字段', 'EXCEL', 'HIS', 1, '[]'),
('病案首页-标准JSON', 'HOMEPAGE_JSON_STD', '病案首页标准JSON导出格式，便于系统对接', 'JSON', 'API', 1, '[]'),
('病案首页-FHIR R4', 'HOMEPAGE_FHIR_STD', '符合HL7 FHIR R4规范的导出格式，包含Patient/Encounter/Procedure资源', 'FHIR', 'EMR', 1, '[]'),
('病案首页-定制Excel(外科)', 'HOMEPAGE_EXCEL_SURG', '外科专用Excel导出模板，重点突出手术相关字段', 'EXCEL', 'HIS', 0, '[]');

-- -----------------------------------------------------------
-- 24. 质控基准表
-- -----------------------------------------------------------
CREATE TABLE quality_benchmark (
    id                      BIGINT          NOT NULL AUTO_INCREMENT COMMENT '主键ID',
    indicator_code          VARCHAR(64)     NOT NULL COMMENT '指标编码',
    indicator_name          VARCHAR(128)    NOT NULL COMMENT '指标名称',
    indicator_category      VARCHAR(32)     NOT NULL COMMENT '指标分类: EFFICIENCY-效率, SAFETY-安全, COST-费用, QUALITY-质量, CLINICAL-临床',
    unit                    VARCHAR(32)     DEFAULT NULL COMMENT '单位',
    benchmark_value         DECIMAL(18,4)   NOT NULL COMMENT '基准值(区域平均)',
    warning_threshold       DECIMAL(18,4)   DEFAULT NULL COMMENT '预警阈值',
    critical_threshold      DECIMAL(18,4)   DEFAULT NULL COMMENT '严重偏离阈值',
    direction               VARCHAR(32)     NOT NULL COMMENT '优劣方向: LOWER_BETTER-越低越好, HIGHER_BETTER-越高越好, RANGE-范围适宜',
    source                  VARCHAR(128)    DEFAULT NULL COMMENT '数据来源(区域质控中心/本院)',
    region                  VARCHAR(128)    DEFAULT NULL COMMENT '区域',
    department              VARCHAR(128)    DEFAULT 'ALL' COMMENT '适用科室: ALL-全部, 或具体科室',
    benchmark_year          INT             NOT NULL COMMENT '基准年份',
    benchmark_quarter       INT             DEFAULT NULL COMMENT '基准季度',
    description             VARCHAR(512)    DEFAULT NULL COMMENT '指标说明',
    sort_order              DECIMAL(10,2)   DEFAULT 999 COMMENT '排序',
    enabled                 TINYINT         DEFAULT 1 COMMENT '是否启用: 0-禁用, 1-启用',
    create_user_id          BIGINT          DEFAULT NULL COMMENT '创建人ID',
    create_user_name        VARCHAR(64)     DEFAULT NULL COMMENT '创建人姓名',
    created_time            DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    ...
    deleted                 TINYINT         NOT NULL DEFAULT 0 COMMENT '逻辑删除',
    PRIMARY KEY (id),
    UNIQUE KEY uk_code_dept_year (indicator_code, department, benchmark_year, deleted),
    KEY idx_indicator_category (indicator_category),
    KEY idx_department (department),
    KEY idx_enabled (enabled),
    KEY idx_benchmark_year (benchmark_year)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='质控基准表(区域医疗质控中心对接)';

-- -----------------------------------------------------------
-- 插入默认质控基准数据(区域质控中心)
-- -----------------------------------------------------------
INSERT INTO quality_benchmark (indicator_code, indicator_name, indicator_category, unit, benchmark_value, warning_threshold, critical_threshold, direction, source, region, department, benchmark_year, description, sort_order, enabled, create_user_name) VALUES
('avg_stay_days', '平均住院日', 'EFFICIENCY', '天', 8.50, 12.00, 15.00, 'LOWER_BETTER', '区域质控中心', '区域', 'ALL', 2025, '患者从入院到出院的平均天数，越低表示效率越高', 1, 1, '系统初始化'),
('antibiotic_rate', '抗生素使用率', 'SAFETY', '%', 30.00, 45.00, 60.00, 'LOWER_BETTER', '区域质控中心', '区域', 'ALL', 2025, '住院患者使用抗菌药物的比例，越低表示合理用药越好', 2, 1, '系统初始化'),
('ssi_rate', '手术部位感染率', 'SAFETY', '%', 1.50, 3.00, 5.00, 'LOWER_BETTER', '区域质控中心', '区域', 'ALL', 2025, '手术患者发生手术部位感染的比例', 3, 1, '系统初始化'),
('avg_total_cost', '平均住院费用', 'COST', '元', 18000.00, 25000.00, 35000.00, 'LOWER_BETTER', '区域质控中心', '区域', 'ALL', 2025, '患者单次住院的平均总费用', 4, 1, '系统初始化'),
('avg_surgery_cost', '平均手术费用', 'COST', '元', 6500.00, 9000.00, 12000.00, 'LOWER_BETTER', '区域质控中心', '区域', 'ALL', 2025, '单次手术的平均费用', 5, 1, '系统初始化'),
('drug_ratio', '药占比', 'COST', '%', 35.00, 45.00, 55.00, 'LOWER_BETTER', '区域质控中心', '区域', 'ALL', 2025, '药品费用占总住院费用的比例', 6, 1, '系统初始化'),
('critical_patient_rate', '危重患者比例', 'QUALITY', '%', 15.00, 25.00, 35.00, 'RANGE', '区域质控中心', '区域', 'ALL', 2025, '危重患者占全部患者的比例，过高或过低均异常', 7, 1, '系统初始化'),
('avg_blood_loss', '平均失血量', 'CLINICAL', 'ml', 250.00, 400.00, 600.00, 'LOWER_BETTER', '区域质控中心', '区域', 'ALL', 2025, '手术过程中的平均失血量', 8, 1, '系统初始化'),
('transfusion_rate', '输血率', 'CLINICAL', '%', 8.00, 15.00, 22.00, 'LOWER_BETTER', '区域质控中心', '区域', 'ALL', 2025, '手术患者接受输血的比例', 9, 1, '系统初始化'),
('reoperation_rate', '非计划再手术率', 'SAFETY', '%', 1.00, 2.50, 4.00, 'LOWER_BETTER', '区域质控中心', '区域', 'ALL', 2025, '因并发症等原因需要再次手术的比例', 10, 1, '系统初始化');
